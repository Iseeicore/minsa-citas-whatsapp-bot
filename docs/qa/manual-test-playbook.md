...
| 3.13m ✅ ⚙ | **Con MINSA real y un día con más de 10 horarios** (tras tocar **Ver más horarios**): con `5:45 PM` visible en la lista, escribir `quiero una cita a las 5`. | Resuelve a `17:45` y pide confirmación, igual que si hubieras escrito solo `a las 5`. | **Causa confirmada** (reproducida con un test específico, no solo la captura): la frase completa lleva la palabra **"una"** antes de "cita" — el mismo `normalize()` que lee "la una" como la 1 en punto convertía CUALQUIER "un"/"una" a "1", incluido el artículo de "una cita". Como el parser toma el primer número de 1-2 dígitos de la frase, agarraba el "1" de "una" antes de llegar al "5" real, y preguntaba por un horario a la 1 que nadie ofreció. Arreglado: "un"/"una" solo se lee como número cuando va justo después de "la"/"las" (`lib/fsm/time-parser.ts`). Confirmado con `escribir solo "a las 5"` (sin "una") SÍ resolvía bien incluso antes del arreglo — la prueba automática que aisló esto está en `time-parser.test.ts` y `handlers-cita.test.ts`. No depende del canal: mismo código en Sandbox y WhatsApp. | ☐ ☐ |
| 3.13n ⚙ | **Con MINSA real:** un día donde algún horario venga con la hora sin cero a la izquierda (ej. MINSA manda `9:30` en vez de `09:30`) — escribir texto libre para un horario BIEN formado de ese mismo día (ej. `a las 5` si hay un `17:45`). | Resuelve normalmente a ese horario, sin importar que otro horario del mismo día tenga el formato raro. | `unpackHoraSlots` (`time-parser.ts`) ya no descarta el día **completo** por una sola entrada mal formada — ahora ignora solo esa entrada puntual. Blindaje encontrado mientras se investigaba 3.13m; no fue la causa de esa captura (ver 3.13m), pero es un caso real que MINSA puede mandar. | ☐ ☐ |
