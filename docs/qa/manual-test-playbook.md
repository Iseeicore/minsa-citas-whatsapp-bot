...
| 3.15m ⚙ | Solo con MINSA real: como primer mensaje pedir una especialidad que ese establecimiento NO ofrece (ej. `necesito cita en neurología` para un distrito donde solo hay Medicina General y Odontología). | Tras el OTP: `No encontramos *Neurología* en este establecimiento. Estas son las especialidades disponibles aquí:` y la lista `Selecciona la especialidad:` con lo que sí hay. | El hint del primer mensaje (`citaEspecialidadHintText`) no matcheó ningún ítem real (`matchEspecialidadHint`, handlers-cita.ts) — antes se descartaba en silencio; ahora se dice explícitamente. Si el hint SÍ matchea, sigue igual que siempre (`Especialidad detectada: …`), sin este mensaje. | ☐ ☐ |
...
| 3.16m | `quiero una cita de medicina general` y `necesito cita en medicina interna` | Como **primer mensaje**, las dos van directo a `¡Hola! Te ayudaremos a agendar tu cita de {especialidad}...`, **sin** mensaje de medicamentos y **sin** `Un momento, estamos revisando tu mensaje…`. | Contraejemplos de falsos positivos de OOS-05. Reconocido sin IA por `ESPECIALIDAD_ROOTS`/`ESPECIALIDAD_PHRASES` (lib/fsm/cita-hints.ts) — ver 3.16q para el resto de la lista. | ☐ ☐ |
| 3.16q | Como primer mensaje, cada una: `necesito cita en urología`, `quiero cita de otorrinolaringología`, `necesito cita en neurología`, `quiero cita de psiquiatría`, `necesito cita en endocrinología`, `quiero cita de reumatología`, `necesito cita en obstetricia`, `quiero cita de cirugía general`, `quiero una cita de medicina familiar`. | Cada una: `¡Hola! Te ayudaremos a agendar tu cita de {Especialidad}...` directo, sin bienvenida ni menú. | Mismo mecanismo que 3.16m; hallado como hueco real en una corrida de `scripts/run-sandbox-playbook.mjs` (`necesito cita en medicina interna` caía al menú genérico porque no estaba en la lista). | ☐ ☐ |
