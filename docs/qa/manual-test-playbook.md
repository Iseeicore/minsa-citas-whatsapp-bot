...
| 3.13m 🔍 ⚙ | **Solo con MINSA real y un día con más de 10 horarios** (tras tocar **Ver más horarios**): con `5:45 PM` visible en la lista, escribir `a las 5`. | **Observado en campo:** `No hay horarios disponibles a esa hora. Elige uno de la lista:` — a pesar de que `5:45 PM` está literalmente en pantalla. | **Bajo investigación, causa no confirmada.** Por código, `a las 5` debería resolver a las 17:00 y encontrar `17:45` (`matchHoraText`/`hourCandidates` en `time-parser.ts` prueban la hora contra TODO el día, no solo la página visible). Sospecha principal: `unpackHoraSlots` (mismo archivo) exige `HH:MM` con cero a la izquierda y descarta el día **completo** si UN SOLO horario no cumple ese formato — y `lib/fsm/minsa.ts:429` toma `hora_inicio` de MINSA con `String(row.hora_inicio)`, sin normalizar (si algún horario temprano del mismo día viene sin cero, ej. `9:30` en vez de `09:30`, se pierde el día entero). No depende del canal: Sandbox y WhatsApp comparten este código exacto, así que reproduce igual en los dos si se usa MINSA real con un día así. Para confirmarlo hace falta el log de esa conversación (ver §5: `traceId`, `turn.note`, y si es posible el `hora_inicio` crudo que devolvió MINSA ese día). | ☐ ☐ |
