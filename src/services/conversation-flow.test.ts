...
describe("createConversationFlowService — verify_code bounded re-entry (D20/D33, Phase 7)", () => {
  it("verified: advances to cita_awaiting_departamento and stores slots.citaBearer", async () => {
    const { sender, calls } = fakeSender();
    const sessionStore = createMemorySessionStore({ logger: fakeLogger() });
    const key = msisdnDigest(FROM_MSISDN, SESSION_KEY_SECRET);
    await sessionStore.save(
    ...
    // "Identidad verificada…" + the first ubicación question together on the
    ...
    const stored = await sessionStore.load(key);
    expect(stored?.state).toBe("cita_awaiting_departamento");
...
describe("createConversationFlowService — Phase 8 end-to-end integration (full Cita identity pipeline)", () => {
  it("main_menu -> agendar_cita -> DNI -> validate_user(valid) -> OTP -> verify_code(verified) -> cita_awaiting_departamento with a bearer token in slots", async () => {
    const { sender, calls } = fakeSender();
    const sessionStore = createMemorySessionStore({ logger: fakeLogger() });
    const key = msisdnDigest(FROM_MSISDN, SESSION_KEY_SECRET);
    ...
    // cita_awaiting_departamento (MVP fix: no throwaway message needed in between).
    await service.process(makeEvent({ text: "654321" }));
    const final = await sessionStore.load(key);
    expect(final?.state).toBe("cita_awaiting_departamento");
